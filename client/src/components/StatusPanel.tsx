...
import { X, Activity, Database, Server, Clock, TrendingUp } from "lucide-react";
...
export default function StatusPanel({ onClose }: StatusPanelProps) {
	...
	useEffect(() => {
		...
		const interval = setInterval(
			fetchStatus,
			parseInt(process.env.NEXT_PUBLIC_STATUS_CHECK_INTERVAL || "60") *
				1000
					...
					<div className="bg-gray-50 rounded-lg p-4">
						...
						<div className="space-y-2 text-sm">
							...
							<div className="flex justify-between">
								...
								<span
									className={`font-medium ${
										status.dataService?.hasData
											? "text-green-600"
											: "text-red-600"
									}`}
								>
									{status.dataService?.hasData ? "Yes" : "No"}
								</span>
							</div>

							{status.dataService?.dataAge && (
								<div className="flex justify-between">
									...
									<span className="font-medium text-gray-900">
										{Math.round(
											status.dataService.dataAge / 1000
										)}
										s
									</span>
								</div>
						...
						<div className="space-y-2 text-sm">
							...
							<div className="flex justify-between">
								...
								<span className="font-medium text-red-600">
									{status.cache?.misses || 0}
								</span>
							</div>

							{status.cache?.hits && status.cache?.misses && (
								<div className="flex justify-between">
									<span className="text-gray-600">
										Hit Rate:
									</span>
					...
					<div className="bg-gray-50 rounded-lg p-4">
						...
						<div className="space-y-2 text-sm">
							...
							<div className="flex justify-between">
								...
								<span className="font-medium text-gray-900">
									{formatMemory(
										status.server?.memory?.heapUsed || 0
									)}
								</span>
							</div>
							...
							<div className="flex justify-between">
								...
								<span className="font-medium text-gray-900">
									{new Date(
										status.server?.timestamp || Date.now()
									).toLocaleTimeString()}
								</span>
							</div>
						</div>
