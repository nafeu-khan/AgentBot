...
const ObservabilityPanel: React.FC = () => {
	...
	const fetchObservabilityData = async () => {
		try {
			...
			const statusResponse = await fetch("/api/observability/status");
	...
	const createTestTrace = async () => {
		try {
			const response = await fetch("/api/observability/test-trace", {
				method: "POST",
				headers: {
			...
			if (result.success) {
				alert(`Test trace created! Run ID: ${result.runId}`);
				if (result.dashboardUrl) {
					window.open(result.dashboardUrl, "_blank");
				}
			} else {
				alert(`Failed to create test trace: ${result.message}`);
			}
		} catch (err) {
			alert(
	...
	const getStatusColor = (enabled: boolean, hasApiKey: boolean) => {
		...
		if (!hasApiKey) return "text-yellow-400";
							...
							<div className="space-y-2 text-sm">
								...
								<div className="flex justify-between">
									...
									<span className="text-blue-400">
										{observabilityData.langsmith.project}
									</span>
								</div>
								<div className="flex justify-between">
									<span className="text-gray-300">
									...
									<span
										className={
											observabilityData.langsmith.tracing
												? "text-green-400"
												: "text-red-400"
										}
									>
						...
						<div className="bg-gray-700 rounded-lg p-3">
							...
							<div className="space-y-2 text-sm">
								<div className="flex justify-between">
									...
									<span className="text-blue-400">
										{observabilityData.environment.nodeEnv}
									</span>
								</div>
								<div className="flex justify-between">
									<span className="text-gray-300">
								...
								<div className="flex items-start space-x-2">
									...
									<div>
										...
										<p className="text-yellow-200 text-sm mt-1">
											To enable full LangSmith
											functionality, please add your API
											key to the backend environment
											variables. Check the
											LANGSMITH_GUIDE.md for setup
											instructions.
										</p>
									</div>
								</div>
